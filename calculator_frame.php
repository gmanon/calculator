...
if(!isset($_GET['title'])){ $_GET['title'] = "Index"; } 
...
<h2 class="title"><?php echo ucwords($_GET['title']);?></h2>
    ...
    <hr>
    <input id='to' name='to' type='button' onClick='changeText("TO")' class='operation' value='TO'/>
	...
	<!-- Calculator buttons -->
    <?php
        $symbols = array('clear'=>'CE','equal'=>'&equals;','help'=>'?','print'=>'$#9113;','time'=>'&#9200;');
                    
        $index_symbols = array('plus'=>'+','minus'=>'-','times'=>'x','division'=>'/','module'=>'%',
        ...
        echo "<input id='period' name='period' type='button' onClick='changeText(".")' class='numberse' value='&period;'/><br/>";
        function calculateSigns($symbols, $class='sign', $function='changeText' )
        {
            
            foreach($symbols as $key=>$value )
                {
                    echo "<input id='$key' name='$key' type='button' onClick='$function($value)' class='$class $key' value='$value'/>\n";
                }
                
        }
        switch($_GET['title']){ 
            case "dates":
                calculateSigns($date_symbols, 'date-sign', 'changeText');
            break;
            
            case "temperature":
                calculateSigns($temperature_symbols, 'temperature-sign','changeText');
            break;
            
            case "measurements":
                calculateSigns($measurement_symbols['space'], 'measurement-sign','changeText');
                
                calculateSigns($measurement_symbols['volunme'], 'measurement-sign','changeText');
                
            break;
            
            default:
                calculateSigns($index_symbols, 'sign','changeText');
            break;
        }
